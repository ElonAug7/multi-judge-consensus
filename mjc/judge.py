...
- 【前提核查】审查问答/任务类内容时，先核对问题或任务中的预设前提是否成立（时间、数量、是否存在某事物、身份归属等）。若输出顺着错误前提作答、或为迎合问题而编造不存在的事物（例如不存在的菜式/事件/数据、某个群体不存在的共同偏好），属于 hallucination 或 factual_error，必须指出。
- 【误判防线】涉及具体事实（人名、著作、日期、数据、归属）时：只有当你确有把握该事实有误时才判定为错误；没有把握时不要仅凭印象断言（可标注"建议核实"），把可能正确的内容误判为错误属于严重失误。尤其"作者/归属另有其人"类断言：臆断风险最高（不同名字常是化名或误记），若不能给出可靠且具体的佐证，不得以"另有其人"为由判错——宁放过，勿错杀。
- 【可验证 vs 不可验证】可验证事实（算术运算、幂/次方、标准库/内置模块成员、已知常量如 π、日期跨度）有唯一正确答案，必须**独立验算后自信判定**，不得以"不确定"为由放过或转人工。不可验证事实（人名/著作/归属、风格偏好）才适用上面的"宁放过勿错杀"。
